...
class SList:
    ...
    def append(self, newNode):
        if self.head is None:  # SList가 빈 리스트인 경우
            ...
            # current.next가 None이면 while 종료, ==> current가 마지막 노트임을 의미
    ...
    def showList(self):
        print("[Head]->",end="")  # 초기 정보를 출력하고 줄바꿈하지않음.(-> 다음 이어질 출력과 연결)
        current = self.head
    ...
    def appendValue(self,value):
        ...
        lst.showList()
#=======================================================================
# 리스트의 첫 노드(head)를 삭제하며 반환한다. (빈리스트이면 None 반환)
    def unshift(self):
...
# 정렬되지 않은 현재의 리스트를 정렬 상태로 재구성한다.
    def sort(self, order="UP"):
        ...
        # 현재 리스트의 노드를 하나씩 차례대로 꺼내서 sorted 리스트에 insertSorted 로 추가한다.
        while self.count:
...
# 리스트의 정렬 상태를 유지하면서 새 노드를 추가한다.
    def insertSorted(self,newNode, order="UP"):
        if self.head is None:
            ...
            # 새 노드가(newNode)가 삽입될 위치를 찾기 위한 준비를 한다.
            current = self.head # 새 노드보다 큰 값을 가진 노드를 차례대로 찾아가기 위한 변수(첫 노드부터 비교 시작)
            previous = None   # current가 다음 노드로 이동할 때 ,현재의 current 값을 백업하며 따라간다.
    ...
    def remove(self,value):
        ...
        if targetNode is None:
            ...
            if previous is None: # targetNode가 리스트의 첫 노드이므로
                self.head = current.next #리스트의 첫 노드(head)sms targetNode의 다음 노드가 된다.
            else:
                previous.next = current.next
            # 마무리 하고 반환값 설정
            #
            current.next = None  # 삭제 대상 노드의 연결 고리(next) 제작
            self.count -= 1
            return current
#=======================================================================
...
slist = SList()
while True:
    value = int(input("새 노드값 : "))
    if value == 0:
        break
    slist.insertSorted(Node(value), order="")
    slist.showList()
#=======================================================================
slist = SList()
